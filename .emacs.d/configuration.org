...
(use-package emacs
  ...
  (project-vc-extra-root-markers '("*.sln" "*.slnx")))
...
(use-package eglot
  ...
  (eglot-code-actions-on-idle nil)
  ;; fsautocomplete loading a large .sln can blow past the default 30s
  (eglot-connect-timeout 90)
  ;; Suppress inline type-hint overlays — too noisy. Toggle per-buffer with
  ;; M-x eglot-inlay-hints-mode if you want them back.
...
(use-package dotnet
  ...
  ;; Enable dotnet-mode for F# (fsharp-mode is remapped to fsharp-ts-mode)
  (fsharp-ts-mode . dotnet-mode)
...
;; Rider-ish "Go to Symbol" over the workspace.
(use-package consult-eglot
  :after (consult eglot)
  :bind (:map eglot-mode-map
              ("C-c s" . consult-eglot-symbols)))

;; Header-line breadcrumb: namespace → class → method.
(use-package breadcrumb
  :hook (prog-mode . breadcrumb-local-mode))

;; Async format-on-save. Formatters (fantomas, dotnet-format, black, …) come
;; from Nix; apheleia just dispatches to them.
...
(use-package highlight-indent-guides
  ...
  :hook (fsharp-ts-mode . highlight-indent-guides-mode))

;; fsharp-mode is needed for the .fs auto-mode-alist entry; major-mode-remap-alist
;; then redirects to fsharp-ts-mode, so no fsharp-mode hooks are needed here.
...
(use-package fsharp-ts-eglot
  ...
  :demand t
  :custom
  ;; server-install-dir and auto-install come from
  ;; modules/dev/dotnet's extraConfig so the Nix store path is pinned.
  (fsharp-ts-eglot-inlay-hints nil)
  :config
  ;; eglot 1.23 calls contact with 0 args; bypass the wrapper and call server-command directly
  (add-to-list 'eglot-server-programs
               '((fsharp-ts-mode fsharp-ts-signature-mode) .
                 (fsharp-ts-eglot-server . fsharp-ts-eglot--server-command))))
